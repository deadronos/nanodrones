...
use the filename convention `DESNNN-example-name.md` for designs (zero-padded 3-digit ID).
...
use the filename convention `TASKNNN-example-name.md` for tasks (zero-padded 3-digit ID).
...
- Tasks & plan: `memory/tasks/_index.md` and `memory/tasks/TASKNNN-example-name.md`
...
> Use the filename pattern `TASKNNN-example-name.md` when creating task files (zero-padded 3-digit ID). Ensure IDs are unique across `memory/tasks` and `memory/tasks/COMPLETED` before assigning a number.
