...
describe('Simulation engine', () => {
  it('advances deterministically for identical inputs', () => {
    ...
    const ctx = { input: { forward: true, backward: false, left: false, right: false }, heading: 0, dt: FIXED_DT };
  ...
  it('assigns mining orders to nearest resource', () => {
    ...
    const assigned = runSimTick({ ...state, orders: [order] }, { input: { forward: false, backward: false, left: false, right: false }, heading: 0, dt: FIXED_DT });

    expect(assigned.orders[0].status === 'assigned' || assigned.orders[0].status === 'completed').toBeTruthy();
